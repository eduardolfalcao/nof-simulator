...
public class MainJournalFtight {
...
public static void main(String [] args){
		...
		double fairnessLowerThreshold = 0.8;	//0.95
		...
		String outputDir = "/home/eduardolfalcao/Área de Trabalho/Dropbox/Doutorado/Disciplinas/Projeto de Tese 3/journal cc elsevier/simulations/goup-i/exp-i/PI-ranging-02-08/";
		...
		int [] numberOfFreeRidersFixedDemandK05 = new int [] {25, 23, 21, 20, 19, 18, 17, 16, 15, 15, 14, 14, 13, 12, 12};
		int [] numberOfCollaboratorsDemandK05 = new int [] {75, 77, 79, 80, 81, 82, 83, 84, 85, 85, 86, 86, 87, 88, 88};
//		
...
//		
		...
		for(int i = 0; i < fixedDemand.length; i++){
			...
			Simulator sim = new Simulator(numPeers, numSteps, pi, new int [] {numberOfCollaboratorsDemandK05[i]}, new int [] {numberOfFreeRidersFixedDemandK05[i]},
					dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
					outputDir+(dynamic?"fdnof-":"sdnof-")+design+"D"+(formatter.format(fixedDemand[i])).replace(",", ".")+kappa05+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
					kappa, design, index,f);
			sim.startSimulation();
			
			kappa = 1;
			pi[0] = variablePIK1[i];
			sim = new Simulator(numPeers, numSteps, pi, new int []{99}, new int [] {1},
					dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
					outputDir+(dynamic?"fdnof-":"sdnof-")+design+"D"+(formatter.format(fixedDemand[i])).replace(",", ".")+kappa1+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
					kappa, design, index,f);
			sim.startSimulation();
			...
			sim = new Simulator(numPeers, numSteps, pi, new int []{99}, new int [] {1},
					dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
					outputDir+(dynamic?"fdnof-":"sdnof-")+design+"D"+(formatter.format(fixedDemand[i])).replace(",", ".")+kappa2+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
					kappa, design, index,f);
			sim.startSimulation();
			...
			sim = new Simulator(numPeers, numSteps, pi, new int []{99}, new int [] {1},
					dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
					outputDir+(dynamic?"fdnof-":"sdnof-")+design+"D"+(formatter.format(fixedDemand[i])).replace(",", ".")+kappa4+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
					kappa, design, index,f);
			sim.startSimulation();
		}
		...
		// PI FIXO e D VARIAVEL
					...
					numberOfFreeRidersFixedDemandK05 = new int [] {16, 19, 22, 24, 27, 29, 31, 33, 35, 36, 38, 40, 41, 43, 44};
...
//					design = "pi-fixo/";
					...
					for(int i = 0; i < fixedPI.length; i++){
						...
						Simulator sim = new Simulator(numPeers, numSteps, pi, numberOfCollaborators, new int [] {numberOfFreeRidersFixedDemandK05[i]},
								dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level,
								outputDir+(dynamic?"fdnof-":"sdnof-")+design+"PI"+(formatter.format(fixedPI[i])).replace(",", ".")+kappa05+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
								kappa, design, index,f);
						sim.startSimulation();
						
						kappa = 1;
						d[0] = variableDK1[i];
						sim = new Simulator(numPeers, numSteps, pi, numberOfCollaborators, new int [] {1},
								dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
								outputDir+(dynamic?"fdnof-":"sdnof-")+design+"PI"+(formatter.format(fixedPI[i])).replace(",", ".")+kappa1+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
								kappa, design, index,f);
						sim.startSimulation();
						...
						sim = new Simulator(numPeers, numSteps, pi, numberOfCollaborators, new int [] {1},
								dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
								outputDir+(dynamic?"fdnof-":"sdnof-")+design+"PI"+(formatter.format(fixedPI[i])).replace(",", ".")+kappa2+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
								kappa, design, index,f);
						sim.startSimulation();
						...
						sim = new Simulator(numPeers, numSteps, pi, numberOfCollaborators, new int [] {1},
								dynamic, nofWithLog, fairnessLowerThreshold, d, capacitySupplied, changingValue, seed, level, 
								outputDir+(dynamic?"fdnof-":"sdnof-")+design+"PI"+(formatter.format(fixedPI[i])).replace(",", ".")+kappa4+"FR"+f+"tau"+fairnessLowerThreshold, pairwise,
								kappa, design, index,f);
						sim.startSimulation();
					}
