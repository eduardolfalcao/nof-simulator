...
public class ExcelReader{
	...
	public static final int replications = 5;
	
	static double[][] fairness = new double [16][replications];
	static 	double[][] collaboratorsSatisfacion = new double [16][replications];
	static double[][] freeRidersSatisfacion = new double [16][replications];
	static double[][] capacitySupplied = new double [16][replications];
    ...
    public static void mountTables(){
  	...
  	String fileName = "/home/eduardolfalcao/Área de Trabalho/Dropbox/Doutorado/Projeto - NoF/NoF/INDEX/Dynamic  (DYNAMIC) - 100 peers - 4000 steps - FREERIDERS.0% freeriders - CONSUMING.0% consuming probability - DEMAND.0 peers demand - 5.0% change value - NoF by SquareRoot.xlsx";
    	
    	
    	boolean [] dynamic = {false, true};
    	int [] consumingStateProbability = {20, 50};
    	int [] percentageCollaborators = {50, 100};
    	int [] peersDemand = {2, 3};
    ...
    public static double[] readValues(String fileName){
    	...
    	double [] mainValues = new double [4];
    	
    	int [] sheetIndexArray = {0,1,2,3};
    ...
    public static void fulfillArrays(double [] mainValues, int linha, int coluna){
    	...
    	ExcelReader.capacitySupplied[linha][coluna] = mainValues[3];	
    ...
    public static void printValues(){
    	...
    	System.out.println("# CAPACITY SUPPLIED #");
    	for(int i = 0; i < capacitySupplied.length; i++){
    		for(int j = 0; j < capacitySupplied[0].length; j++)
    			System.out.print(String.format("%.10f", capacitySupplied[i][j])+"	");
    		System.out.println();
    	}
